...
Adapted by Marie D. Mulder 2025
...
#    OPS.update(kwargs)
    ...
    #V11
    Dfmt_11 = ['i', 'i', 'i', '256S', '2i', 'i', 'i', 'i', '2i', 'f', 'f', 'i', 'i', 'f', 'f', '2i', 'i']
    if bf:
#        V11 = False
...
#            print('j = ', j) #MDM
#            try:
#                 h[I[j]] = a[j].decode('ascii')
#            except:
#                 h[I[j]] = a[j]
#            print(h[I[j]])
##            if h[I[j]]  == 'Version 11.0':
#            if np.any(h[I[j]] == 'Version 11.0'):
#                V11 = True
#                bf.seek(0)
#                #print (h[I[j]],I[j])
#                break
#        if V11 == True:
        a = [bf.read(fmt) for fmt in Dfmt_11]
        for j in range(len(a)):
            h[I[j]] = a[j]
#            print('h[I[j]] = a[j] ', h[I[j]], ' = ', a[j])
            #print (h[I[j]],I[j])
        ...
        for i in range(h['nspec']):
            one = getbin('i'); # wd input skipped 
            wd = getbin('c', 10);  # wd input skipped 
            skip();
            one = getbin('i'); # dd input skipped 
            dd = getbin('c', 10);  # dd input skipped
            skip();
            h['numzgrid'].append(getbin('i'))
            h['species'].append(''.join([getbin('c').decode('ascii') for i in range(10)]))
            skip();
        h['numpoint'] = getbin('i')
        ...
        for i in range(h['numpoint']):
            ...
            sp = ''
            while re.search(r"\w", getbin('c').decode('ascii')): #collect the characters for the compoint
                bf.seek(-1, 1)
                sp = sp + getbin('c').decode('ascii')
        ...
        ny = h['numygrid']
        Dfmt = ['f'] * nx 
        h['oro'] = np.zeros((nx, ny), np.float64)
        skip()
        for ix in range(nx):
        ...
        nz = h['numzgrid'][0]
        Heightnn = np.zeros((nx, ny, nz), np.float64)
        for ix in range(nx):
            if OPS.ltopo == 1:
                Heightnn[ix, :, 0] = Z[ix, :]
            else:
    ...
    for i in h.available_dates:
        #print (int(i[:4]), int(i[4:6]), int(i[6:8]), int(i[8:10]), int(i[10:12]), int(i[12:]))
