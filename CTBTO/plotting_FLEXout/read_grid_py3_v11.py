    ...
    def _dumpgrid(dmp_i, cnt_r, dmp_r, grd, k, nage, nx, ny):
        ...
        for ir in range(cnt_r):
            ...
            if conc:
                ...
                kz = int(n / (H.numxgrid * H.numygrid))
                jy = int((n - kz * H.numxgrid * H.numygrid) / H.numxgrid)
                ix = int(n - H.numxgrid * H.numygrid * kz - H.numxgrid * jy)
#                print('dumpgrid: n,ix,jy,kz,k,nage', n,ix,jy,kz,k,nage)
...
#                print( grd[0,0,0,0,0])
                ...
                if dmp_r[ir] * fact > 0:
                    ...
                    #pos = pos + 1
                jy = int(n / H.numxgrid)
                ix = int(n - H.numxgrid * jy)
#                H_numxgrid = H.numxgrid #MDM
#                print('dumpgrid: n, H.numxgrid', n, H_numxgrid) #MDM
#                print('dumpgrid: ix,jy,k,nage', ix,jy,k,nage) #MDM
    ...
    wetgrid = np.zeros((H.numxgrid, H.numygrid, H.numpointspec, 1), np.float64)
    drygrid = np.zeros((H.numxgrid, H.numygrid, H.numpointspec, 1), np.float64)
    datagrid = np.zeros((H.numxgrid, H.numygrid, H.numzgrid[0], H.numpointspec, nage), np.float64)
    #f = file(filename,'rb')
    #print( filename)
